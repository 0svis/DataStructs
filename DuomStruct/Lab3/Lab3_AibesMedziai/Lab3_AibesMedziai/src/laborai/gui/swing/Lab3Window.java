...
import laborai.Lab3Zilinskas.GreitaveikaRies;
import laborai.Lab3Zilinskas.RiesutGamyba;
import laborai.Lab3Zilinskas.Riesutas;
...
public class Lab3Window extends JFrame implements ActionListener {
    ...
    private static SortedSetADTx<Riesutas> riesSet;
    private static SortedSetADTx<Riesutas> riesSetSubSet;
    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            ...
            if (source.equals(panButtons.getButtons().get(0))) {
                ...
                MedzioIlgumas();
            }else if (source.equals(panButtons.getButtons().get(7))
                    || source.equals(panButtons.getButtons().get(8))) {
    ...
    public void treeGeneration(String filePath) throws MyException {
        ...
        if (filePath == null) {
            riesArray = RiesutGamyba.generuotiIrIsmaisyti(sizeOfGenSet, sizeOfInitialSubSet, coef);
            panParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            riesSet.load(filePath);
            riesArray = new Riesutas[riesSet.size()];
            int i = 0;
            for (Object o : riesSet.toArray()) {
                riesArray[i++] = (Riesutas) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(riesArray), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        riesSet.clear();
        for (Riesutas a : riesArray) {
            riesSet.add(a);
        }
        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, riesSet.toVisualizedString(delimiter),
    ...
    private void treeAdd() throws MyException {
        Riesutas ries = RiesutGamyba.gautiIsBazes();
        riesSet.add(ries);
        panParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, ries, MESSAGES.getString("msg7"));
        KsSwing.oun(taOutput, riesSet.toVisualizedString(delimiter));
        KsSwing.setFormatStartOfLine(false);
    ...
    private void treeRemove() {
        ...
        if (riesSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(riesSet.size());;
            Riesutas ries = (Riesutas) riesSet.toArray()[nr];
            riesSet.remove(ries);
            KsSwing.oun(taOutput, ries, MESSAGES.getString("msg6"));
            KsSwing.oun(taOutput, riesSet.toVisualizedString(delimiter));
            //getHeight();
        }
        KsSwing.setFormatStartOfLine(false);
    ...
    private void treeIteration() {
        ...
        if (riesSet.isEmpty()) {
            ...
            KsSwing.oun(taOutput, riesSet, MESSAGES.getString("msg8"));
        }
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void treeEfficiency() throws MyException {
        ...
        GreitaveikaRies gt = new GreitaveikaRies();
    ...
    private void readTreeParameters() throws MyException {
        ...
        try {
            ...
            maxSk = Double.valueOf(panParam1.getParametersOfTable().get(++i).replace("-", "x"));
    ...
    private void brangiausias(){
        Riesutas ries = new Riesutas();
        double kaina = 0.0;
        double didz = maxSk;
        for(Riesutas s: riesSet){
            if(s.getKgKaina() > kaina && s.getKgKaina() < didz){
                kaina = s.getKgKaina();
                ries = s;
            }
        }
        KsSwing.oun(taOutput, ries, MESSAGES.getString("msg9"));
        //KsSwing.oun(taOutput, riesSet.toVisualizedString(delimiter));

    }

    private void MedzioIlgumas(){
        int n = riesSet.MedzioDydis();
        KsSwing.oun(taOutput, n, MESSAGES.getString("msg10"));

        //int nr =  new Random().nextInt(riesSet.size());
        Riesutas ries = (Riesutas) riesSet.toArray()[1];
        Riesutas ries1 = (Riesutas) riesSet.toArray()[5];
        riesSet.subSet(ries, ries1);
        KsSwing.oun(taOutput, riesSet, "Sub set ");
    }
    private void subSetCreation(){
        int indexStart;
        int indexEnd;
        String indexString = panParam1.getTfOfTable().get(4).getText();
        String[] parts = indexString.split(" ");
        indexStart = Integer.parseInt(parts[0]) - 1;
        indexEnd = Integer.parseInt(parts[1]) - 1;
        Riesutas coinStart = (Riesutas) riesSet.toArray()[indexStart];
        Riesutas coinEnd = (Riesutas) riesSet.toArray()[indexEnd];
        riesSetSubSet = (SortedSetADTx<Riesutas>) riesSet.subSet(coinStart,coinEnd);
        KsSwing.oun(taOutput, riesSetSubSet, MESSAGES.getString("msg8"));


    }
    private void containCheck(){
        for(Riesutas ries : riesSet){
            KsSwing.oun(taOutput, ries + ": " + riesSetSubSet.contains(ries));
        }
    }
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectedIndex()) {
            case 0:
                riesSet = new BstSetKTUx(new Riesutas());
                break;
            case 1:
                riesSet = new AvlSetKTUx(new Riesutas());
                break;
            default:
                enableButtons(false);
