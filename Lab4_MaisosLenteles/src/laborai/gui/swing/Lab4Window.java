...
import Lab4Sambaravicius.Serialas;
import Lab4Sambaravicius.SerialuGamyba;
import Lab4Sambaravicius.GreitaveikosTyrimas;
...
public class Lab4Window extends JFrame implements ActionListener {
    ...
    private MapADTx<String, Serialas> map;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private final SerialuGamyba serialuGamyba = new SerialuGamyba();

    private Serialas[] serArray;
    ...
    private void initComponents() {
        ...
        panButtons = new Panels(new String[]{
            ...
            MESSAGES.getString("button7")}, 1, NUMBER_OF_BUTTONS);
    ...
    public void mapGeneration(String filePath) {
        ...
        // Jei failas nenurodytas - generuojami automobiliai ir talpinami atvaizdyje
         serArray = serialuGamyba.gamintiIrZiuretiSerialus(sizeOfGenSet, sizeOfInitialSubSet);
        if (filePath == null) {
            for (Serialas a : serArray) {
                map.put(
                        serialuGamyba.gautiIsBazesSerId(),//raktas
                        a);
            }
    ...
    public void mapAdd() throws MyException {
        Serialas a = serialuGamyba.ziuretiSerialus();
        map.put(
                serialuGamyba.gautiIsBazesSerId(),// Raktas
                a);
        table.setModel(new TableModel(map.getModelList(panParam1.getTfOfTable().get(5).getText()),
    ...
    public void avgLengthEmptyCount(){
        double vid = map.getAvgLength();
        int sk = map.getEmptyCount();
        KsSwing.oun(taEvents, String.format("~%.3f",vid), MESSAGES.getString("msg7"));
        KsSwing.oun(taEvents, sk, MESSAGES.getString("msg8"));
    ...
    public void mapEfficiency() {
        ...
        new Thread(() -> {
            try {
                String result;
                while (!(result = (String) gt.getResultsLogger().take())
    ...
    private void remove(){
        List<JTextField> tfs = panParam3.getTfOfTable();
    ...
    private void search(){
        List<JTextField> tfs = panParam3.getTfOfTable();
        String searchKey = tfs.get(0).getText();
        Serialas s = map.get(searchKey);

        if(s == null){
            KsSwing.oun(taEvents, "tokio serialo nėra");
            return;
        }
        KsSwing.oun(taEvents, "Rastas serialas " + s);
    }
    private void searchContainsValue(){
        List<JTextField> tfs = panParam3.getTfOfTable();
        String pav = tfs.get(1).getText();
        Serialas ser = new Serialas();
        for(Serialas s : serArray){
            if(s.getPavadinimas().equals(pav)){
                ser = s;
            }
        }
        boolean isInMap = map.containsValue(ser);
        if(!isInMap){
            KsSwing.oun(taEvents, "Serialo nėra");
            return;
        }
        KsSwing.oun(taEvents, "Serialas yra");
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>(new String(), new Serialas(), initialCapacity, loadFactor, ht);
                break;
            case 1:
                map = new HashTablex<>(new String(), new Serialas(), initialCapacity, loadFactor, ht);
                break;
